...
from typing import Dict, Optional, TYPE_CHECKING, NamedTuple, Any
from ray.rllib.policy import Policy

number_bound, start = 3, 0
rews, all_actions, r = [], [], 0
array_indexes = np.arange(start, mip_task.m, number_bound)
...
class SampleCallback(DefaultCallbacks):
    ...
    def on_postprocess_trajectory(
        ...
        if total_reward > self.best_reward and len(actions[rewards >= 0]) == self.number_bound:
            actions = actions[rewards >= 0]
            total_reward = np.sum(actions * self.coefs)
...
def train(ubound: int, c: np.ndarray, b: np.ndarray, p: np.ndarray, m: int, n: int, bound: int, steps_per_forward: int,
          array: np.ndarray = array_indexes, R=r):
    ray.shutdown()
    ray.init()
    ...
    for ind in range(len(array) - 1):
        ...
        config["env_config"] = {'ubound': ubound, 'c': constr, 'b': bound, 'p': coefs, 'm': number_bound, 'n': n}
