...
class MyEnv(gym.Env, ABC):
    def __init__(self, env_config: dict):
        ...
        self.observation_space = gym.spaces.Dict({
            ...
            'mask': gym.spaces.MultiBinary(self.ubound + 1)})
        self.state = {'rem': np.array(self.b), 'j': 0, 'mask': gym.spaces.MultiBinary(self.ubound + 1).sample()}
        self.done = False
        self.reward = 0
    ...
    def step(self, action):
        ...
        if np.any(rem < 0):
            ...
            j += 1
            self.state = {'rem': rem, 'j': j, 'mask': gym.spaces.MultiBinary(self.ubound + 1).sample()}
...
class Agent(TFModelV2):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, true_obs_shape=mip_task.n,
                 action_vec_size=mip_task.ubound + 1, *args, **kwargs):
...
def train(ubound: int, c: np.ndarray, b: np.ndarray, p: np.ndarray, m: int, n: int, bound: int, steps_per_forward: int):
    ...
    register_env(MyEnv, 'custom_env', env_config={'ubound': ubound, 'c': c, 'b': b, 'p': p, 'm': m, 'n': n})
    trainer_config = {
        "model": {
